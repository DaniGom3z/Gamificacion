...
export class UsuarioLogroPrismaRepository implements UsuarioLogroRepository {
  ...
  async findUsuarioAggregate(
    _idUsuario: UsuarioId
  ): Promise<UsuarioAggregate | null> {
    return null;
  }

  async saveUsuarioAggregate(
    usuarioAggregate: UsuarioAggregate
  ): Promise<void> {
    console.log(
      "Guardando aggregate del usuario:",
      usuarioAggregate.id.getValue()
    );
